...
cover: https://stock-demo-image.nenchan.cn/website/ai/sleep-money.png
...
所以我设计了一个**签到**的功能，每天签到赠送对话次数，没错，就是这么朴实无华。我也是在这个时候才发现那些大厂设计签到日历、抽奖转盘等功能背后的强大，每当我们签到、抽奖时，都在展示广告，对大厂来说，很多类目已经到了白热化厮杀的阶段，比如 TB、PDD 再取获取新用户肯定是很难的，但是现有用户让他们每天进来看一下，肯定是相对容易的。只是把原本的推广费用，挪一部分出来个用户作为签到奖励而已。
...
这就是我说的`尽量让产品朝着自己运营目标方向前进，`而不是做一些不知所以的开发。`每一项功能、运营策略都要有目的性，并且最终监控运作结果。将这些策略、结果变成自己沉淀下来的经验，在下一个产品上能用，这才是我希望达到的状态。`
